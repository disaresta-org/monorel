...
Where a subpackage's directory name collides with a stdlib or widely-known third-party package (e.g. `internal/forge/github`), the package name follows the directory name. Disambiguation happens at import sites with an alias (`gh "github.com/disaresta-org/monorel/internal/forge/github"`) or by the implementation type being unexported (callers see only `forge.Client`).
...
When a constructor can fail at runtime (missing env, invalid arg from config), pair `New` with a `Build`-style sibling, not just one or the other. monorel's surface is small; today only `forge` providers use this (`github.New(ctx, opts) (forge.Client, error)`). When you add another constructor that can fail, follow the same pattern.
...
Default to (1) or (2). The `forge.Fake` is (3) (mutex-backed). `git.Exec` is (4) — one working tree per Exec instance, no concurrent invocations.
...
Don't generalize across providers past the shared helpers. Each forge has its own auth flow, error shapes, and pagination quirks; the repetition between provider impls is honest and abstraction would just move it.
...
Validate at the public API boundary (`config.Load`, `forge.New`, CLI flag parsing), then trust the input through internal code. Don't re-check `nil` on every internal hop.
...
- **Fakes**: `forge.NewFake()` and `git.NewFake()` for higher-layer tests that don't need a real working tree or live API.
...
`forge.Fake.FailNext` is a `func() error` consulted before every API call. Use `forge.FailOnce(err)` for a one-shot failure; use `forge.FailOnNth(n, err)` to test partial-success paths.
