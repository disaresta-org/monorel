...
| `path` | string | required | Package directory relative to the repo root. Used to locate the package's `go.mod`, whose module directive determines the module major (a `/v3` path plans tags at `v3.x.y`). |
