...
//
// Returns the slice of created [provider.Release]s. On the first failure
...
func PublishReleases(ctx context.Context, c provider.Client, res *Result) ([]*provider.Release, error) {
	if res == nil {
		return nil, fmt.Errorf("release: nil result")
	}

	out := make([]*provider.Release, 0, len(res.Releases))
